...
const tipsArr=[
  ...
  "Original game is From GitHub",
  "Try X ray Ultimate in Minecraft",
  "CJ follow the train",
  "Another Versions From Destiny is not original"
];
const tipsDiv=document.getElementById("tips");
setInterval(()=>{tipsDiv.innerText=tipsArr[Math.floor(Math.random()*tipsArr.length)];},2000);

let maxHP=3, gameOver=false, speed=2, score=0, gameSpeed=1, crazyMode=false, message="", messageTime=0, gameStarted=false;
...
function randomColor(){let l="0123456789ABCDEF";let c="#";for(let i=0;i<6;i++){c+=l[Math.floor(Math.random()*16)];}return c;}
...
function drawEnemyBullets(){ctx.fillStyle=crazyMode?randomColor():"#f80";enemyBullets.forEach(b=>{ctx.fillRect(b.x,b.y,b.w,b.h);b.y+=5*gameSpeed;}); enemyBullets=enemyBullets.filter(b=>b.y<450);}
...
function showMessage(text){message=text; messageTime=Date.now();}
function checkCollisions(){enemies.forEach((e,ei)=>{bullets.forEach((b,bi)=>{if(b.x<e.x+e.w && b.x+b.w>e.x && b.y<e.y+e.h && b.y+b.h>e.y){enemies.splice(ei,1);bullets.splice(bi,1);score++;doomShoot.volume=volume;doomShoot.play();}}); if(player.x<e.x+e.w && player.x+player.w*player.fatMultiplier>e.x && player.y<e.y+e.h && player.y+player.h*player.fatMultiplier>e.y){player.hp--;enemies.splice(ei,1);doomKill.volume=volume;doomKill.play(); if(player.hp<=0)gameOver=true;}});}
...
function drawMiniMap(){mctx.fillStyle="#000";mctx.fillRect(0,0,225,90);mctx.fillStyle="#ff0";mctx.fillRect(player.x/2,70,5,5); mctx.fillStyle="#f00";enemies.forEach(e=>{mctx.fillRect(e.x/2,e.y/5,4,4);}); mctx.fillStyle="#ff0";bullets.forEach(b=>{mctx.fillRect(b.x/2,b.y/5,2,2);});}
function shootBullet(){bullets.push({x:player.x+23,y:player.y,w:5,h:10,color:player.shootColor});doomShoot.volume=volume;doomShoot.play();}
function loop(){ 
  if(gameOver) return;
  ctx.clearRect(0,0,600,450);
  drawGround(); drawPlayer(); drawBullets(); drawEnemyBullets(); drawEnemies(); checkCollisions(); drawText(); drawMiniMap();
  if(message && Date.now()-messageTime<3000){ctx.fillStyle="#0ff";ctx.font="18px monospace";ctx.fillText(message,50,420);}
  requestAnimationFrame(loop);
}
function spawnMeteor(){let m=document.createElement("div"); m.classList.add("meteor"); m.style.width=(5+Math.random()*15)+"px"; m.style.height=(5+Math.random()*15)+"px"; m.style.left=Math.random()*window.innerWidth+"px"; m.style.top="-20px"; body.appendChild(m); let speed=2+Math.random()*3; let interval=setInterval(()=>{let t=parseFloat(m.style.top); m.style.top=(t+speed)+"px"; if(t>window.innerHeight){clearInterval(interval); m.remove();}},30);}
...
// JSON Mods
const modInput = document.getElementById("modFile");
document.getElementById("loadModBtn").onclick = () => modInput.click();
modInput.onchange = async (e) => {
  ...
  } catch { showMessage("Invalid Mod ❌"); }
};
function applyMod(mod){
  if (typeof mod.playerSpeed==="number") gameSpeed = Math.min(Math.max(mod.playerSpeed,0.5),3);
  if (typeof mod.playerHP==="number") player.hp=Math.min(mod.playerHP,10);
  if (typeof mod.crazyMode==="boolean") crazyMode=mod.crazyMode;
  if (typeof mod.playerColor==="string") player.color=mod.playerColor;
}

// Cheats
document.getElementById("cheatBtn").onclick = ()=>{
  let code=prompt("Enter cheat code: SD31 XX8L | D4KJ 7A2M | CL91 VV1K");
  if(code=="SD31 XX8L"){score*=2;showMessage("Score Multiplied!");}
  else if(code=="D4KJ 7A2M"){player.hp=999;showMessage("Invincible!");}
  else if(code=="CL91 VV1K"){gameSpeed*=2;showMessage("Speed Boost!");}
  else showMessage("Invalid Code");
};
